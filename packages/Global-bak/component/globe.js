...
const DAT = {
  Globe: function (container, opts) {
    ...
    const target = { x: Math.PI * 3 / 2, y: Math.PI / 6.0 }
    ...
    function init () {
      ...
      uniforms.texture.value = THREE.ImageUtils.loadTexture(Earth) // 材质
      // uniforms.texture.value = THREE.TextureLoader(Earth) // 材质
    ...
    function addData (data, opts) {
      ...
      if (opts.format === 'magnitude') {
        ...
        // throw ('error: format not supported: ' + opts.format) // should be thrown
      }
      if (opts.animated) {
      ...
      if (this._baseGeometry !== undefined) {
        if (this.is_animated === false) {
          ...
          if (this._baseGeometry.morphTargets.length < 8) {
            ...
            const padding = 8 - this._baseGeometry.morphTargets.length
